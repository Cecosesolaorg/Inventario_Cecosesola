document.addEventListener('DOMContentLoaded', () => {
    ...
    const initialProducts = [
        "ACEITE AGROIL *12", "ACEITE MI ACEITE *12", "ACEITE NATUROIL", "ACEITE PORTUMESA", "ACEITE VATEL DE LITRO *12",
        "ATUN DIPLOMATICO *48", "ATUN PACIFICO *48", "ATUN PEÑERO AZUL *48", "ATUN PEÑERO VERDE *12", "AVENA DE VITA",
        "AVENA DON JORGE", "AVENA QUAKER", "CAFÉ CORDILLERA 200GRMS", "CAFÉ CORDILLERA 500 GRMS", "CAFÉ EN GRANO 250GR DULCE AMAR",
        "CAFÉ EN GRANO 400GRMS", "CAFÉ EN GRANO COROMOTANA", "CAFÉ LO NUESTRO", "CAFÉ MATILDE", "CAFÉ VARINA",
        "CARNE DE SOYA", "FORORO VALLE HONDO", "HARINA DE AVENA QUAKER", "HARINA DE TRIGO ESPECIAL LEUDANTE",
        "HARINA DE TRIGO ESPECIAL TODO USO", "HARINA DE TRIGO MARY", "HARINA TRIGO DULCE MAR", "LECHE LA CAMPIÑA",
        "LECHE MONTAÑA FRESCA", "LECHE VALLELAC", "SARDINA", "SARDINA INCOSA", "SARDINA VEMPOR", "TAMARINDO EL ESFUERZO"
    ];

    // Cargar lista de productos (personalizada + base)
    let products = JSON.parse(localStorage.getItem('productsList_P2')) || initialProducts;
                    ...
                    if (choice === "1") {
                        ...
                        if (newName && newName.trim() && newName.trim().toUpperCase() !== p) {
                            ...
                            if (index !== -1) {
                                ...
                                localStorage.setItem('inventoryData', JSON.stringify(inventoryData));
                                localStorage.setItem('productsList_P2', JSON.stringify(products));
                                renderTable();
                            }
                        }
                    } else if (choice === "2") {
    ...
    if (downloadBtn) {
        downloadBtn.onclick = async () => {
            ...
            row2.values = ["PASILLO Nº 2 CAFE 🛒 DE FERIA DEL ESTE 🛒"];
            ...
            const hasChecklistItems = Object.values(inventoryData).some(p => p.checkState === 1 || p.checkState === 2);
            ...
            // --- RESPALDO AUTOMÁTICO EN CLOUD (Firebase) ---
            try {
                const aisleName = document.querySelector('.aisle-title').textContent.trim();
                const today = new Date().toISOString().split('T')[0];
                const backupPath = `respaldos/${today}/${aisleName}/${responsable.toUpperCase()}`;
                
                firebase.database().ref(backupPath).set({
                    fecha: new Date().toLocaleString(),
                    responsable: responsable,
                    companero: companero,
                    asistente: assistantName,
                    productos_contados: totalInventoried,
                    datos: inventoryData
                });
                console.log("Respaldo enviado a la nube correctamente.");
            } catch (e) {
                console.error("Error al enviar respaldo:", e);
            }
