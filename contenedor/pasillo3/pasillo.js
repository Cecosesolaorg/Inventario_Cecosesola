document.addEventListener('DOMContentLoaded', () => {
    ...
    const initialProducts = [
        "ADOBO MEL BUEN", "AFRECHO", "ALIÑOS CURCUMA", "CACHITOS", "CATALINA BLANCA", "CATALINA NEGRA",
        "CHOCOLATE ARTESANAL", "COCADAS", "FORORO MEL BUEN", "MERMELADA", "MIEL CAMPO REAL", "MIEL SAN CARLOS",
        "PAN ARABE", "PAN DE DIOS", "PAN GUAYABA", "PAN HAMBURGUESA", "PAN INTEGRAL", "PAN SANDWICH GRANDE",
        "PAN SANDWICH NACIONAL", "PAN SANDWICH PEQUEÑO", "PAN TUNJA", "PASTELITOS", "SALSA MONCAR",
        "SOPA MAGGY", "TAMARINDO ETNA", "VAINILLA"
    ];

    // Cargar lista de productos (personalizada + base)
    let products = JSON.parse(localStorage.getItem('productsList_P3')) || initialProducts;
                    ...
                    if (choice === "1") {
                        ...
                        if (newName && newName.trim() && newName.trim().toUpperCase() !== p) {
                            ...
                            if (index !== -1) {
                                ...
                                localStorage.setItem('inventoryData', JSON.stringify(inventoryData));
                                localStorage.setItem('productsList_P3', JSON.stringify(products));
                                renderTable();
                            }
                        }
                    } else if (choice === "2") {
    ...
    if (downloadBtn) {
        downloadBtn.onclick = async () => {
            ...
            row2.values = ["PASILLO Nº 3 PANES 🛒 DE FERIA DEL ESTE 🛒"];
