document.addEventListener('DOMContentLoaded', () => {
    ...
    let products = JSON.parse(localStorage.getItem('productsList_P10')) || initialProducts;
                    ...
                    if (choice === "1") {
                        ...
                        if (newName && newName.trim() && newName.trim().toUpperCase() !== p) {
                            ...
                            if (index !== -1) {
                                ...
                                localStorage.setItem('inventoryData', JSON.stringify(inventoryData));
                                localStorage.setItem('productsList_P10', JSON.stringify(products));
                                renderTable();
                            }
                        }
                    } else if (choice === "2") {
    ...
    document.getElementById('btnSumarCant').onclick = () => {
        try {
            ...
            if (!isNaN(result)) {
                ...
                // --- PREGUNTA KILOS O GRAMOS (Solo para QUESO) ---
                let label = "";
                if (activeProduct.toUpperCase().includes("QUESO")) {
                    const choice = prompt("¿Qué está sumando?\n1. Kilos (kg)\n2. Gramos (g)", "1");
                    if (choice === "1") label = "kg";
                    else if (choice === "2") label = "g";
                }
    ...
    if (downloadBtn) {
        downloadBtn.onclick = async () => {
            ...
            row2.values = ["PASILLO Nº 10 CAVA CUARTO 🛒 DE FERIA DEL ESTE 🛒"];
