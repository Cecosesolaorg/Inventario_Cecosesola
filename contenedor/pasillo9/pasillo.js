document.addEventListener('DOMContentLoaded', () => {
    ...
    let products = JSON.parse(localStorage.getItem('productsList_P9')) || initialProducts;
                    ...
                    if (choice === "1") {
                        ...
                        if (newName && newName.trim() && newName.trim().toUpperCase() !== p) {
                            ...
                            if (index !== -1) {
                                ...
                                localStorage.setItem('inventoryData', JSON.stringify(inventoryData));
                                localStorage.setItem('productsList_P9', JSON.stringify(products));
                                renderTable();
                            }
                        }
                    } else if (choice === "2") {
    ...
    document.getElementById('btnSumarCant').onclick = () => {
        try {
            ...
            if (!isNaN(result)) {
                ...
                // --- PREGUNTA KILOS O UNIDADES (Solo para productos pesados) ---
                const heavyKeywords = ["PESADA", "PESADO", "KG", "KILO", "GRS", "GR", " G "];
                let label = "";
                if (heavyKeywords.some(key => activeProduct.toUpperCase().includes(key))) {
                    const choice = prompt("¿Qué está sumando?\n1. Kilos (kg)\n2. Unidades (und)", "1");
                    if (choice === "1") label = "kg";
                    else if (choice === "2") label = "und";
                }

                if (isOperationOnTotal) {
                    newTotal = result;
                    historyStep = ` (${currentExpression.replace(/ANS/g, prevTotal)})${label} = ${newTotal}`;
                } else {
                    newTotal = prevTotal + result;
                    historyStep = ` + ${currentExpression}${label} = ${newTotal}`;
                }

                if (tempHistory === "0" || tempHistory === "") {
                    if (isOperationOnTotal) {
                        tempHistory = `${currentExpression.replace(/ANS/g, prevTotal)}${label} = ${newTotal}`;
                    } else {
                        tempHistory = `${currentExpression}${label} = ${newTotal}`;
                    }
                } else {
    ...
    if (downloadBtn) {
        downloadBtn.onclick = async () => {
            ...
            row2.values = ["PASILLO Nº 9 GRANOS 🛒 DE FERIA DEL ESTE 🛒"];
